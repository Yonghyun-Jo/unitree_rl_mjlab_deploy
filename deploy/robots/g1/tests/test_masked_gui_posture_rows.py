...
그래서 여기선 GUI 를 띄우지 않고 viser 를 가짜로 바꿔 main() 을 한 번 돌려 다음을 본다:
  ...
  ② 한 줄에 masked_gui.BTNS_PER_ROW 개 이하      (한 줄로 되돌아가면 다시 잘린다)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if per_row > MAX_PER_ROW:
            print(f"FAIL: BTNS_PER_ROW={per_row} > {MAX_PER_ROW} — 패널 폭을 넘어 뒤쪽 자세가 잘린다")
            return 1
