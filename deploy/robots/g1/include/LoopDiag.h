...
public:
    ...
    void reset() {
        n_ = 0; overrun_ = 0; ticks_ = 0; elapsed_s_ = 0.0;
    ...
    void tick(double work_ms, double period_ms) {
        if (n_ < CAP) { work_[n_] = work_ms; period_[n_] = period_ms; ++n_; }
    ...
    int format(char* buf, int cap, const char* const* seg_names) const {
        ...
        int k = std::snprintf(buf, cap,
            ...
            hz, q(w, n, 0.50), q(w, n, 0.95), w[n - 1], budget_ms_, overrun_, ticks_,
            np ? q(p, np, 0.95) : 0.0, np ? p[np - 1] : 0.0);
