...
# 모드가 바뀐 뒤 이 시간 안에 나가는 쓰기에도 mode_req 를 다시 싣는다. 창은 «CSV 시각» [s] 이다.
#   1회성 요청은 파일에 «마지막 쓰기» 로만 남는다 — 50 Hz 재생이 20 ms 뒤 다음 명령으로 덮으면 50 Hz 로 읽는
#   제어기가 한 번도 못 볼 수 있다. 창 안의 쓰기가 전부 mode_req 를 실으므로, 모드 변경부터 창 뒤 첫 쓰기까지
#   파일에 놓인 프레임은 «연달아» mode_req 프레임이다 — 그 연속 구간이 CSV 시각으로 ≥ 이 시간을 덮는다
#   (마지막 한 프레임이 그만큼 남는다는 뜻이 아니다). 재생은 표본 시각에 맞춰 자므로 보통 벽시계도 같다 —
#   밀려서 sleep 없이 몰아 보낼 때만 벽시계로 더 짧아질 수 있다.
MODE_REQ_HOLD_S = 0.2
