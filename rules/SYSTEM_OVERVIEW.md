...
- shm 폴링: `State_Mimic.cpp` 의 `g_poll_gui()`·`g_poll_vr()` — 매 제어 스텝(정책 스레드). `g_poll_gui()` 는 `g_poll_inputs()` 첫 줄에서, `g_poll_vr()` 는 정책 루프가 `g_poll_inputs()` 다음에 직접 부른다. 줄 번호는 자주 바뀌니 함수 이름으로 찾을 것.
- 시뮬/로봇 연결: `./g1_ctrl --network=<iface>` (`main.cpp:37` ChannelFactory Init).
  - **sim2sim = `lo`**, **실로봇 = 실제 iface(예: enp5s0)**.
- 키보드 백업(터미널 포커스): `1/2/3/4`=mode(표의 `key` 열), `[`/`]`=재생할 클립 고르기(**재생 중에도 된다 — 직립일 때만**. 0번 칸 «stand» 로 돌아오는 것은 언제나), `5`/`6`=표엔 있으나 지금 슬롯이 모르는 모드 → 거부 로그, `config/mode5_keys.yaml` 의 키(지금 표 순서 = 진입 로그 순서로 `z h x c b n` = 직립·크랩·네발·포복·앉기·눕기)=mode5 자세(**mode5 에서만** — 다른 모드에선 한 줄 안내만), `WASD/QE`=속도, `p`=정지, `v`=Velocity, `m`=Mimic_Masked.
  mode4(와 mode5)는 **직립일 때만**(골반 높이 추정 `z_fk ≥ 0.65 m` ∧ 걸러진 기울기 `< 30°`) 떠날 수 있다 — 아니면 요청이 거부되고 `[cmd_mode] … 거부: …` 한 줄이 남는다(계약 v1 슬롯의 mode4 도 같다. mode5 에서 직립 버튼 `z` 의 유지 상태 없이 들어갈 수 있는 곳은 via_ground 모드(지금은 예약된 mode6)뿐이고, 나머지(1·2·3·4 — mode4 의 ground_capable 포함)는 전부 그 유지 상태 ∧ 직립을 요구한다).
  mode4 는 **들어갈 때도 직립이어야 한다**(표의 `exit: upright` 가 진입 조건도 된다) — mode1 에서 무릎이 무너져 앉은 채 `4` 를 누르면 `[cmd_mode] … 거부: 직립에서만 들어간다` (바닥에서 클립 재생을 켜면 스스로 일어서는 명령이 없고 이탈도 거부돼 출구가 `p` 뿐이었다). 그리고 **Mimic 체류는 클립 재생 모드로 시작하지 않는다**: 지난 체류가 mode4 로 끝났으면(넘어짐 → Passive → `p→f→m`, 또는 Velocity 를 거쳐 Dance1 로) 진입 때 폴백 모드(mode1)로 내린다 — 로그 `[mode] 직전 모드 4 (playback) 는 스스로 일어서는 명령이 없는 바닥 모드다… -> mode1 로 내린다`. mode5 는 재진입해도 mode5(직립 버튼으로 시작).
  안전층도 GroundCapable 모드(지금 4·5, 6 은 예약)에선 다르다(`SafetyPolicy.h`, 계약 v1 슬롯의 mode4 포함): **낮거나 기운 자세**(`z_fk < 0.65` ∨ 기울기 `≥ 30°`)에서 qd_warn 이 걸리면 폴백 모드 대신 **Passive** 로 간다(로그 `qd_warn LATCHED … -> Passive. 복귀=p→f→m` — 복귀는 `p` → `f` → `m`). 넘어짐 판정(기울기 `> 57.3°` → Passive)의 관문은 명령의 종류로 갈린다. **mode4(클립 재생)** 는 재생 클립의 현재 프레임 **골반 기울기가 `< 57.3°`** 이면 건다 — 높이도 최근 이력도 묻지 않는다(클립은 매 틱 전신 자세를 명령하는 참조라 «일어나는 중» 이 없다). 지금 클립들은 골반 기울기가 전 구간 39° 이하라(깊이 앉는 demo6 구간 포함 — 골반 < 0.65 m 가 9.2 %) **mode4 는 늘 판정된다 = B2 이전과 같은 보호**. 골반을 57.3° 넘게 눕히는 클립 구간이 생기면 그 구간에서만 꺼진다(그사이 넘어지면 이 판정은 못 잡고, mode4 이탈은 직립에서만이라 출구는 `p` — qd_warn·qd_crit·E-stop 은 그대로). **mode5** 는 **명령이 직립이고**(자세의 목표 높이 `≥ 0.65 m`) **최근 1 s 안에 섰을** 때만 건다. «섰음» = 명령이 직립인 동안(mode1~3 은 정의상 직립 명령이라 늘 채운다 — 넘어지는 도중 4·5 로 바꿔도 판정이 이어진다) `z_fk ≥ 0.65` ∧ 원시·걸러진 기울기 둘 다 `< 57.3°` 이고, 명령이 낮아지면 그 기억을 지운다. 그래서 서 있다 넘어지면 잡고, 일부러 내려가기·누운 데서 일어나기(엉덩이부터 드는 기립 포함)는 넘어짐으로 치지 않는다. **낮은 자세를 눌렀다 직립 버튼으로 되돌리면**, 로봇이 다시 «섰음»(`z_fk ≥ 0.65` ∧ 기울기 `< 57.3°`)이 될 때까지 넘어짐 판정이 꺼져 있다 — 그사이 넘어지면 이 판정은 못 잡는다(qd_warn·qd_crit·E-stop 은 그대로). mode6 은 명령에 자세가 없어 들어오면 넘어짐 판정을 **아예** 안 건다(`rules/ADDING_A_MODE.md` 2절). mode1~3 은 종전 그대로(넘어짐 판정 항상 · qd_warn → 폴백 모드).

### 2.2 IPC 채널 (파일 계약 — Python↔C++ 레이아웃 동기 필수)
| 파일 | magic | 내용 | 쓰는 쪽 | 읽는 쪽 |
|---|---|---|---|---|
| `/dev/shm/g1_masked_gui` | 0x6704 (v2, 48 B) | 1회성 `mode_req` + base_vel + gait params + 1회성 mode5 자세(`m5_preset`, `m5_press_seq`) + 1회성 `clip_req` | `tools/gui_shm.py`(masked_gui / pico_control_bridge / replay_cmd) | `g_poll_gui()` |
...
- **GUI 채널 v2 (2026-09-22)** — 바이트 배치는 `tools/gui_shm.py` `FMT` ↔ `State_Mimic.cpp` `struct GuiCtrl` 을 `tests/test_gui_shm_layout.py` 가 대조한다(실행기에 포함).
  - `mode_req` = **1회성** 모드 요청(0 = 요청 없음). 쓰는 쪽이 한 번 보내고 0 으로 되돌린다 → 속도 슬라이더·자세 버튼만 움직여서는 모드가 다시 요청되지 않는다(v1 은 매 쓰기마다 `cmd_mode` 를 다시 보내, 키보드로 바꾼 모드가 GUI 쪽 모드로 되돌아갔다). 같은 모드 재요청은 제어기가 조용히 수락한다 — 못 받았으면 다시 누르면 된다.
  - **Mimic 진입마다 기준선**: `enter()` 뒤 첫 `g_poll_gui` 는 파일이 유효한 v2 면 `seq`(와 자세가 실린 프레임이면 `m5_press_seq`)를 기준으로만 잡고 **아무것도 적용하지 않는다** — 모드·자세·클립 요청도, 속도·foot-gen 도(한 줄 `[gui] 진입 기준선 …`). 비유한 값이 든 프레임이어도 같다(기준으로 삼키고 적용하지 않는다). 그래서 Passive·FixStand·지난 세션에서 누른 것이 `m` 직후 발동하지 않고, 키보드로 멈춘 뒤 `p→f→m` 해도 GUI 에 남은 옛 속도로 걷지 않는다. 파일이 없거나 v2 가 아니면(옛 0x6701 파일 등) 기준선 없이 **첫 v2 쓰기가 그대로 먹는다**(옛 GUI 가 남긴 파일 때문에 새 PICO 브리지의 첫 누름이 삼켜지지 않게). `m` 뒤에 GUI 로 누른 것부터 먹는다.
  - 🔴 **모든 Mimic 체류는 속도 명령 0 에서 시작한다**(키보드·GUI·PICO·replay 로 준 속도, 컨트롤러의 전환 램프·저역통과까지) — **재진입(`p→f→m`) 뒤엔 속도를 다시 줄 것.** 계약 v1 슬롯도 같다(전엔 지난 체류의 속도가 이어졌다: 넘어져 Passive 인 동안 스틱을 놓아도 그 «정지» 프레임은 기준선에 삼켜져, 스틱이 가운데인데 옛 속도로 걸었다).
  - 실수 칸(vx·vy·wz·turn_k·height_scale)에 NaN·inf 가 하나라도 있으면 그 프레임을 통째로 버린다(`[gui seq=N] 거부: 비유한 값 …` 한 줄).
  - `m5_preset` = 0 없음, 1.. = `Mode5Presets.h` 표 index + 1, **1회성**(누른 프레임에만 — 유실된 누름이 나중 쓰기에 늦게 발동하지 않게) · `m5_press_seq` = 누를 때마다 +1(같은 자세 재입력 = 새 목표. GUI 는 시각(ms)으로 시작해 재시작해도 번호가 안 겹친다). 제어기는 **자세가 실린 프레임(`m5_preset ≠ 0`)만** 누름·기준으로 본다 — 자세 칸이 0 인 PICO·replay 프레임이 기준을 옮겨 GUI 의 마지막 자세를 «다시 누르는» 일이 없다. **mode5 에서만** 먹는다. 브라우저 GUI 는 **키가 있는 자세만**(`config/mode5_keys.yaml`, 지금 6개) 버튼으로 내고, 제어기도 키 없는 자세 번호는 거부한다(`[m5] gui «…» — 키 없는 자세는 아직 안 연다`) — 표의 나머지 자세는 검증 뒤에 연다.
  - `clip_req` = −1 그대로, 0.. = 고를 클립 칸(`g_build_clips` 순서 **0 = stand** · primary · light · demo6 중 실린 것). **1회성**, 재생 중 교체는 직립(골반 ≥ 0.65 m ∧ 기울기 < 30°)일 때만 — 0번 «stand» 로 돌아오는 것만 언제나(키보드 `[`/`]` 와 같은 문 `g_select_clip`). 브라우저 GUI 의 클립 버튼·모드 단축키는 원천에서 만든다(클립 = `config/config.yaml` `Mimic_Masked` 의 `motion_file*` 중 실린 것, 모드 = 생성된 `tools/mode_table_gen.py` 의 키).
  - **요청할 수 있는 모드가 채널마다 다르다**: GUI = 표의 모든 모드(슬롯 지원·이탈 조건은 `ModeRuntime` 이 본다) · VR(`g1_vr_ref`) = 직립 모드(`safety: upright_only`)만.
  - ⚠ **VR 브리지는 모드를 매 프레임 요청한다**(새 `seq` 마다 자기 `cmd_mode` 2/3) — 켜 둔 채 mode5 에 들어가면 직립 hold 가 되는 순간(진입 ~0.5 s 뒤) 그 요청이 받아들여져 저절로 2/3 으로 나가고, mode4 는 직립이면 곧바로 끌려 나가 머물지 못한다. mode4·5 를 쓸 땐 VR 브리지(`g1_vr_ref` 쓰는 쪽)를 끈다.
...
- 모드는 **X/Y/A 를 누를 때마다** 1회성 `mode_req` 로 요청한다 — 브리지가 이미 그 모드라고 알고 있어도 보낸다(같은 모드면 제어기가 조용히 수락). 1회성 요청은 유실될 수 있으므로(Mimic 진입 기준선 · 50 Hz 쓰기가 한 틱 안에 덮음) **안 먹었으면 같은 버튼을 다시 누르면 된다.** 스틱을 움직여도 모드는 다시 요청되지 않는다(v1 은 매 쓰기마다 되돌렸다).
...
- 넘어짐(기울기 `> 57.3°`) 판정은 이 층이 아니라 FSM `registered_checks` 의 `bad_orientation` → Passive 다
  ...
  원시 기울기). 모드별 관문은 `SafetyPolicy.h`: UprightOnly(1·2·3)는 항상 적용, 클립 재생(4)은 클립 현재
  프레임 골반 기울기 `< 57.3°` 일 때(이력 안 봄), mode5 는 «명령 직립 ∧ 최근 1 s 에 섰음» 일 때만(§2.1 끝).
  관문은 policy_thread 가 매 틱 계산해 원자값으로 넘긴다.
